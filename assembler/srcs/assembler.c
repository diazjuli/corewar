...
/*   Updated: 2019/01/26 12:00:29 by tholzheu         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../includes/assembler.h"

int					get_op(char *mnemonic, t_vars *ob) // compare the mnemonic with the table and get the op_code - 1
{
	int i;
...
int				main(int argc, char **argv)
{
	...
	if (lexer(&ob, fd) == -1) // checks that everything is lexicographically correct
		return (-1);
	close(fd);
	fd = open(argv[1], O_RDONLY); // close it and open it from the beginning again
	get_label_address(&ob, fd);
	close(fd);
	fd = open(argv[1], O_RDONLY);
	ob.output_fd = open(set_output_name(argv[1]), O_CREAT | O_RDWR | O_TRUNC, 0644);
